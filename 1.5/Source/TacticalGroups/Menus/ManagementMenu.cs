...
namespace TacticalGroups
{
	...
	public class ManagementMenu : TieredFloatMenu
	{
        ...
        public override void DoWindowContents(Rect rect)
		{
			...
			Rect hostilityResponseRect = new Rect(rect.x + 10, rect.y + 55, 24, 24);
			HostilityResponseModeUtilityGroup.DrawResponseButton(hostilityResponseRect, colonistGroup, true);
			Rect medicalCareRect = new Rect(hostilityResponseRect.xMax, hostilityResponseRect.y, 24, 24);
			MedicalCareUtilityGroup.MedicalCareSelectButton(medicalCareRect, colonistGroup);

			if (ModsConfig.IdeologyActive)
			{
				Rect groupColorRect = new Rect(medicalCareRect.xMax, medicalCareRect.y, 24, 24);
			...
			Rect timeAssignmentSelectorGridRect = new Rect(rect.x + 5, rect.y + 20, 150, 65);
			...
			DoApparelPolicyHeader(outfitHeaderRect);

			check = new Vector2(outfitHeaderRect.xMax - 26f, outfitHeaderRect.yMax - 33);
			Widgets.Checkbox(check, ref colonistGroup.groupApparelPolicyEnabled);
			drawBoxRect = new Rect(check, new Vector2(24f, 24f));
			Widgets.DrawBox(drawBoxRect);
			TooltipHandler.TipRegion(drawBoxRect, Strings.GroupApparelPolicyTooltip);

			Rect outfitRect = new Rect(rect.x + policyButtonWidth + 30, rect.y + 205, policyButtonWidth, 30f);
			DoApparelPolicyCell(outfitRect);
			...
			Widgets.Checkbox(check, ref colonistGroup.groupFoodPolicyEnabled);
			drawBoxRect = new Rect(check, new Vector2(24f, 24f));
			Widgets.DrawBox(drawBoxRect);
		...
		public void DoApparelPolicyHeader(Rect rect)
		{
			MouseoverSounds.DoRegion(rect);
			...
			{
				var window = new Dialog_ManageApparelPolicies(null);
				Find.WindowStack.Add(window);
			}
			UIHighlighter.HighlightOpportunity(rect2, "ManageApparelPolicies");
		}

		public void DoApparelPolicyCell(Rect rect)
		{
			...
			Widgets.Dropdown(rect2, colonistGroup, (ColonistGroup group) => group.pawns.First().outfits.CurrentApparelPolicy, Button_GenerateMenu,
				colonistGroup.pawns.First().outfits.CurrentApparelPolicy.label.Truncate(rect2.width), null,
				colonistGroup.pawns.First().outfits.CurrentApparelPolicy.label, null, null, paintable: true);
			x += rect2.width;
			x += 4f;
			...
			{
				var window = new Dialog_ManageApparelPolicies(colonistGroup.pawns.First().outfits.CurrentApparelPolicy);
				Find.WindowStack.Add(window);
			}
			x += num2;
		}

		private IEnumerable<Widgets.DropdownMenuElement<ApparelPolicy>> Button_GenerateMenu(ColonistGroup group)
		{
			foreach (ApparelPolicy outfit in Current.Game.outfitDatabase.AllOutfits)
			{
				yield return new Widgets.DropdownMenuElement<ApparelPolicy>
				{
					option = new FloatMenuOption(outfit.label, delegate
					{
						...
						if (group.groupApparelPolicyEnabled)
						{
							group.groupApparelPolicy = outfit;
						}
						if (!(ModCompatibility.assignManagerSaveCurrentStateMethod is null))
						{
		...
		public void DoFoodHeader(Rect rect)
		{
			...
			{
				var window = new Dialog_ManageFoodPolicies(null);
				Find.WindowStack.Add(window);
			}
		}
		...
		{
			DoAssignFoodPolicyButtons(rect, colonistGroup);
		}

		private IEnumerable<Widgets.DropdownMenuElement<FoodPolicy>> Button_GenerateFoodMenu(ColonistGroup group)
		{
			foreach (FoodPolicy foodRestriction in Current.Game.foodRestrictionDatabase.AllFoodRestrictions)
			{
				yield return new Widgets.DropdownMenuElement<FoodPolicy>
				{
					option = new FloatMenuOption(foodRestriction.label, delegate
					{
						...
						if (group.groupFoodPolicyEnabled)
						{
							group.groupFoodPolicy = foodRestriction;
						}
					}),
		...
		private void DoAssignFoodPolicyButtons(Rect rect, ColonistGroup group)
		{
			int num = Mathf.FloorToInt((rect.width - 4f) * 0.714285731f);
			int num2 = Mathf.FloorToInt((rect.width - 4f) * 0.2857143f);
			float x = rect.x;
			Rect rect2 = new Rect(x, rect.y + 2f, num, rect.height - 4f);
			Widgets.Dropdown(rect2, group, (ColonistGroup g) => g.pawns.First().foodRestriction.CurrentFoodPolicy, Button_GenerateFoodMenu,
				group.pawns.First().foodRestriction.CurrentFoodPolicy.label.Truncate(rect2.width), null,
				group.pawns.First().foodRestriction.CurrentFoodPolicy.label, null, null, paintable: true);
			x += num;
			x += 4f;
			if (Widgets.ButtonText(new Rect(x, rect.y + 2f, num2, rect.height - 4f), "AssignTabEdit".Translate()))
			{
				var window = new Dialog_ManageFoodPolicies(group.pawns.First().foodRestriction.CurrentFoodPolicy);
				Find.WindowStack.Add(window);
			}
			x += num2;
		}
