...
namespace TacticalGroups
{
    ...
    public class ColonistGroup : IExposable, ILoadReferenceable
	{
            ...
            if (Event.current.type == EventType.MouseDown)
            {
				...
				if (Event.current.button == 0)
				{
					if (Event.current.clickCount == 1)
					{
						...
						if ((WorldRendererUtility.WorldSelected && Map != null) || (Map != null && Map != Find.CurrentMap))
						{
							if (ActivePawns.Any())
							{
		...
		{
			if (groupFoodPolicy != null)
			{
				pawn.foodRestriction.CurrentFoodPolicy = groupFoodPolicy;
			}

			if (groupApparelPolicy != null)
			{
				pawn.outfits.CurrentApparelPolicy = groupApparelPolicy;
			}
			...
			{
				pawn.playerSettings.AreaRestrictionInPawnCurrentMap = groupArea;
			}
		...
		public void ActivatePreset(GroupPreset preset)
		{
			...
			if (preset.groupFoodPolicy != null)
			{
				groupFoodPolicy = null;
				groupFoodPolicyEnabled = true;
			}
			if (preset.groupApparelPolicy != null)
			{
				groupApparelPolicy = preset.groupApparelPolicy;
				groupApparelPolicyEnabled = true;
			}
		...
		public void ResetGroupPolicies()
		{
			...
			groupFoodPolicy = null;
			groupFoodPolicyEnabled = false;
			groupApparelPolicy = null;
			groupApparelPolicyEnabled = false;
		}
		public virtual void ExposeData()
		{
			...
			Scribe_Values.Look(ref groupFoodPolicyEnabled, "groupFoodPolicyEnabled");
			Scribe_Values.Look(ref groupApparelPolicyEnabled, "groupApparelPolicyEnabled");
			try
			{
			...
			try
			{
				Scribe_References.Look(ref groupFoodPolicy, "groupFoodPolicy");
			}
			catch
			{
				groupFoodPolicyEnabled = false;
			...
			try
			{
				Scribe_References.Look(ref groupApparelPolicy, "groupApparelPolicy");
			}
			catch
			{
				groupApparelPolicyEnabled = false;
		...
		public bool groupApparelPolicyEnabled;
		...
		public bool groupFoodPolicyEnabled;
