...
namespace TacticalGroups
{
	public class MainFloatMenu : TieredFloatMenu
	{
		...
		public override void DrawExtraGui(Rect rect)
		{
			...
			if (!colonistGroup.isColonyGroup)
			{
				...
				if (Mouse.IsOver(disbandPawnRect))
				{
					...
					if (Event.current.type == EventType.MouseDown && Event.current.button == 0 && Event.current.clickCount == 1)
					{
						...
						{
							if (colonistGroup.pawns.Contains(pawn))
							{
			...
			Rect addPawnRect = new Rect(disbandPawnRect.xMax + 15, disbandPawnRect.y, Textures.AddPawnIcon.width, Textures.AddPawnIcon.height);
