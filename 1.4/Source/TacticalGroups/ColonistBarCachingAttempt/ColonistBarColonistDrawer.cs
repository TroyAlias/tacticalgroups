...
namespace TacticalGroups
{
	...
	public class ColonistBarColonistDrawer
	{
		...
		[TweakValue("0CG")] public static bool enableCombine;
		public void DrawColonist(Rect rect, Pawn colonist, Map pawnMap, bool highlight, bool reordering)
		{
			if (enableCombine)
			{
				DrawColonistCombineInt(rect, colonist, pawnMap, highlight, reordering);
			}
			else
			{
				DrawColonistInt(rect, colonist, pawnMap, highlight, reordering);
			}
		}
		...
		public static Dictionary<Pawn, MappedValue> cachedTextures = new Dictionary<Pawn, MappedValue>();
		private void DrawColonistCombineInt(Rect rect, Pawn colonist, Map pawnMap, bool highlight, bool reordering)
		{
			...
			if (cachedTextures.TryGetValue(colonist, out var value))
			{
				GUI.DrawTexture(value.rect, value.texture);
			}
			else
            {
				var cachedTexture = TacticalColonistBar.BGTex;
				if (colonist.needs != null && colonist.needs.mood != null)
				{
					...
					{
						var moodBarTexture = GetMoodBarTexture(colonist);
						cachedTexture = TexturesUtils.GetMergedTexture(rect, position, cachedTexture, Textures.AwakeIcon);
						if (!TacticalGroupsSettings.DisplayBreakRiskOverlay)
						{
							cachedTexture = TexturesUtils.GetMergedTexture(rect, position, cachedTexture, Textures.ColorMoodBarOverlay);
						}
					}
					else
					{
						cachedTexture = TexturesUtils.GetMergedTexture(rect, position, cachedTexture, MoodBGTex);
					}
					...
					if (TacticalGroupsSettings.DisplayBreakRiskOverlay)
					{
						...
						{
							cachedTexture = TexturesUtils.GetMergedTexture(rect, rect.ContractedBy(2f), cachedTexture, Textures.ColorMoodBarOverlayRedMajor);
						}
						else if (colonist.needs.mood.CurLevel < colonist.mindState.mentalBreaker.BreakThresholdMinor)
						{
							cachedTexture = TexturesUtils.GetMergedTexture(rect, rect.ContractedBy(2f), cachedTexture, Textures.ColorMoodBarOverlayRedMinor);
						}
				...
				GUI.DrawTexture(rect, cachedTexture);
				//cachedTextures[colonist] = new MappedValue { rect = rect, texture = cachedTexture };
			}
			...
			//var pawnTexture = PortraitsCache.Get(colonist, pawnTextureSize, Rot4.South, PawnTextureCameraOffset, PawnTextureCameraZoom * TacticalGroupsSettings.PawnScale);
			//GUI.DrawTexture(pawnIconRect, pawnTexture);
		...
		private void DrawColonistInt(Rect rect, Pawn colonist, Map pawnMap, bool highlight, bool reordering)
		{
			if (!(ModCompatibility.alteredCarbonDrawColonist_PatchMethod is null))
			{
				bool prefixValue = (bool)ModCompatibility.alteredCarbonDrawColonist_PatchMethod.Invoke(this, new object[]
			...
			if (colonist.needs != null && colonist.needs.mood != null)
			{
				...
				if (TacticalGroupsSettings.DisplayColorBars)
				{
					GUI.DrawTexture(position, Textures.AwakeIcon);
					if (!TacticalGroupsSettings.DisplayBreakRiskOverlay)
					{
