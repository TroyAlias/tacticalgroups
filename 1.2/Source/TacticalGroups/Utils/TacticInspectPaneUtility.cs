...
{
	public static class TacticInspectPaneUtility
	{
			...
			if (pane.AnythingSelected)
			{
				try
				{
					...
					if (pane.ShouldShowSelectNextInCellButton)
					{
						...
						if (Widgets.ButtonImage(rect2, SelectOverlappingNext))
						{
							pane.SelectNextInCell();
						}
						lineEndWidth += 24f;
		...
		public static InspectTabBase OpenTab(Type inspectTabType)
		{
			...
			if (mainTabWindow_Inspect.CurTabs != null)
			{
				...
				if (list != null)
				{
					for (int i = 0; i < list.Count && !FindCustom((InspectTabBase)list[i]); i++)
					{
					}
				}
				else
				{
					...
					{
						while (enumerator.MoveNext() && !FindCustom(enumerator.Current))
						{
						}
					}
				}
			...
			bool FindCustom(InspectTabBase t)
			{
