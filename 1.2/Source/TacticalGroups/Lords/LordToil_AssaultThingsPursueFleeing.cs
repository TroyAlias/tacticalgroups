	...
	public class LordToil_AssaultThingsPursueFleeing : LordToil
	{
		...
		public override void UpdateAllDuties()
		{
			for (int i = 0; i < lord.ownedPawns.Count; i++)
			{
				...
				if (duty == null || duty.def != TacticDefOf.AssaultThingsFleeing || duty.focus.ThingDestroyed)
				{
					if (!things.Where((Thing t) => t?.Spawned ?? false).TryRandomElement(out Thing result))
					{
						break;
					}
					lord.ownedPawns[i].mindState.duty = new PawnDuty(TacticDefOf.AssaultThingsFleeing, result);
				}
			}
		}
