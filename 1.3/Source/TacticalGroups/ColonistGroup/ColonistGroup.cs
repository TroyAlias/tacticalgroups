...
namespace TacticalGroups
{
    ...
    public class ColonistGroup : IExposable, ILoadReferenceable
    {
        ...
        public virtual void Add(Pawn pawn)
        {
            ...
            {
                pawns.Add(pawn);
                pawnIcons[pawn] = new PawnIcon(pawn, entireGroupIsVisible ? true : false);
        ...
        public void ChangeWorkState(WorkTypeEnum workTypeEnum)
        {
            ...
            if (workTypes.Any())
            {
                foreach (KeyValuePair<WorkType, WorkState> data in workTypes)
                {
                    ...
                    {
                        activeWorkTypes[data.Key] = (WorkState)((int)state + 1);
                    }
                }
            }
            else
        ...
        {
            if (ActiveWorkTypes.Any())
            {
                if (ActiveWorkTypes.Any(x => x.Value == WorkState.ForcedLabor))
                {
                    activeWorkState = WorkState.ForcedLabor;
                }
                else if (ActiveWorkTypes.Any(x => x.Value == WorkState.Active))
                {
