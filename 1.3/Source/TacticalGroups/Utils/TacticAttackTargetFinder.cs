...
{
    public static class TacticAttackTargetFinder
    {
        ...
        public static IAttackTarget BestAttackTarget(IAttackTargetSearcher searcher, TargetScanFlags flags, Predicate<Thing> validator = null, float minDist = 0f,
            float maxDist = 9999f, IntVec3 locus = default(IntVec3), float maxTravelRadiusFromLocus = float.MaxValue,
            bool canBash = false, bool canTakeTargetsCloserThanEffectiveMinRange = true, CombatSearchMode combatSearchMode = CombatSearchMode.Default)
        {
            ...
            {
                tmpTargets.Clear();
                List<IAttackTarget> targets = searcherThing.Map.attackTargetsCache.GetPotentialTargetsFor(searcher);

                if (combatSearchMode == CombatSearchMode.Default)
                {
                    tmpTargets.AddRange(targets);
                }
                else if (combatSearchMode == CombatSearchMode.Strongest)
                {
                    tmpTargets.AddRange(targets.OrderByDescending(x => (x.Thing as Pawn)?.kindDef.combatPower));
                }
                else if (combatSearchMode == CombatSearchMode.Weakest)
                {
                    tmpTargets.AddRange(targets.OrderBy(x => (x.Thing as Pawn)?.kindDef.combatPower));
                }
                else if (combatSearchMode == CombatSearchMode.PursueFleeing)
                {
            ...
            IAttackTarget attackTarget2 = (IAttackTarget)GenClosest.ClosestThingReachable(searcherThing.Position, searcherThing.Map, ThingRequest.ForGroup(ThingRequestGroup.AttackTarget),
                PathEndMode.Touch, TraverseParms.For(searcherPawn, Danger.Deadly, TraverseMode.ByPawn, canBash), maxDist, (Thing x) => innerValidator((IAttackTarget)x), null, 0,
                (maxDist > 800f) ? (-1) : 40);
