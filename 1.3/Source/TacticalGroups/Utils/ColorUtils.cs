...
namespace TacticalGroups
{
    ...
    public static class ColorUtils
    {
        ...
        public static Color? GetDesiredColor(Pawn pawn, Apparel apparel)
        {
            ...
            {
                foreach (ColonistGroup group in groups.SortPawnGroupFirst())
                {
                    if (group.groupColor?.bodyColors != null)
                    {
                        if (apparel.def.IsHeadgear() && group.groupColor.bodyColors.TryGetValue(BodyColor.Head, out ColorOption headColor))
                        {
                            return headColor.GetColor(pawn);
                        }
                        else if (apparel.def.IsFeetGear() && group.groupColor.bodyColors.TryGetValue(BodyColor.Feet, out ColorOption feetColor))
                        {
                            return feetColor.GetColor(pawn);
                        }
                        else if (apparel.def.IsLegsGear() && group.groupColor.bodyColors.TryGetValue(BodyColor.Legs, out ColorOption legsColor))
                        {
                            return legsColor.GetColor(pawn);
                        }
                        else if (apparel.def.IsTorsoGear() && group.groupColor.bodyColors.TryGetValue(BodyColor.Torso, out ColorOption torsoColor))
                        {
                            return torsoColor.GetColor(pawn);
                        }
                        else if (apparel.def.IsArmsGear() && group.groupColor.bodyColors.TryGetValue(BodyColor.Hands, out ColorOption armsColor))
                        {
                            return armsColor.GetColor(pawn);
                        }
                        else if (group.groupColor.bodyColors.TryGetValue(BodyColor.All, out ColorOption allColor))
                        {
        ...
        public static Color? GetDesiredHairColor(Pawn pawn)
        {
            if (pawn.TryGetGroups(out HashSet<ColonistGroup> groups))
            {
                foreach (ColonistGroup group in groups.SortPawnGroupFirst())
                {
                    if (group.groupColor?.bodyColors != null)
                    {
                        if (group.groupColor.bodyColors.TryGetValue(BodyColor.Hair, out ColorOption hairColor))
                        {
