...
namespace TacticalGroups
{
    ...
    public class ColonistBarDrawLocsFinder
    {
        ...
        private static float MaxColonistBarWidth => UI.screenWidth - 520f;
        ...
        private float FindBestScale(out bool onlyOneRow, out int maxPerGlobalRow)
        {
            float num = 1f;
            List<TacticalColonistBar.Entry> entries = TacticUtils.TacticalColonistBar.Entries;
            int colonistCount = entries.Where(x => x.pawn != null).Count();
            int num2 = CalculateGroupsCount();
            float scaleMultiplier = 1f;
            if (!TacticalGroupsSettings.HideGroups)
            {
                int count = TacticUtils.AllColonyGroups.Count;
                count += TacticUtils.AllCaravanGroups.Count;
                if (!WorldRendererUtility.WorldRenderedNow && count > 0)
                {
                    ColonyGroup activeColony = TacticUtils.AllColonyGroups.Where(x => x.Map == Find.CurrentMap).FirstOrDefault();
                ...
                scaleMultiplier += count / 10f;
            }
            float num3 = ((TacticalColonistBar.BaseSize.x + TacticalGroupsSettings.ColonistBarSpacingX) * num);
            float num4 = (MaxColonistBarWidth - (num2 - 1) * 25f * num) / scaleMultiplier;
            int initialValue = Mathf.FloorToInt(num4 / num3);
            while (true)
            {
                num3 = ((TacticalColonistBar.BaseSize.x + TacticalGroupsSettings.ColonistBarSpacingX) * num);
                num4 = (MaxColonistBarWidth - (num2 - 1) * 25f * num) / scaleMultiplier;
                int pawnRowCount = GetPawnRowCount(colonistCount);
                ...
                if (TryDistributeHorizontalSlotsBetweenGroups(maxPerGlobalRow))
                {
                    ...
                    for (int i = 0; i < entries.Count; i++)
                    {
                        if (num5 != entries[i].group)
                        {
                            ...
                            int num6 = Mathf.CeilToInt(entriesInGroup[entries[i].group] / (float)GetHorizontalSlotsPerGroup(entries[i].group));
                            if (num6 > 1)
                            {
                                onlyOneRow = false;
                            }
                            if (TacticalGroupsSettings.OverridePawnRowCount && (GetHorizontalSlotsPerGroup(entries[i].group) * num) >= initialValue)
                            {
                                flag = false;
                                break;
                            }
        ...
        public int GetHorizontalSlotsPerGroup(int group)
        {
            ...
            {
                List<TacticalColonistBar.Entry> entries = TacticUtils.TacticalColonistBar.Entries;
                int pawnCount = entries.Where(x => x.group == group && x.pawn != null).Count();
                if (pawnCount / (float)((horizontalSlotsPerGroup[group])) > GetPawnRowCount(pawnCount))
                {
        ...
        private void CalculateDrawLocs(List<Rect> outDrawLocs, float scale, bool onlyOneRow, int maxPerGlobalRow)
        {
            ...
            float num4 = (num * num3 + (num2 - 1) * 25f * scale);
            if (!TacticalGroupsSettings.HideGroups)
            {
                if (!WorldRendererUtility.WorldRenderedNow)
                {
                    ColonyGroup activeColony = TacticUtils.AllColonyGroups.FirstOrDefault(x => x.Map == Find.CurrentMap);
                    if (activeColony != null)
                    {
            ...
            int num5 = -1;
            int num6 = -1;
            float num7 = (UI.screenWidth - num4) / 2f;
            ...
            for (int j = 0; j < entries.Count; j++)
            {
                if (num5 != entries[j].group)
                {
                    ...
                    {
                        num7 += 25f * scale;
                        num7 += GetHorizontalSlotsPerGroup(num5) * scale * (TacticalColonistBar.BaseSize.x + TacticalGroupsSettings.ColonistBarSpacingX);
                    }
                    if (!TacticalGroupsSettings.HideGroups)
                    {
                        ...
                        else if (entries[j].colonyGroup != null)
                        {
                            if (entries[j].colonyGroup.Map == Find.CurrentMap)
                            {
                                if (!WorldRendererUtility.WorldRenderedNow)
                                {
                                    List<PawnGroup> list = TacticUtils.GetAllSubGroupFor(entries[j].colonyGroup);
                                    if (list.Any())
                                    {
                                        list.Reverse();
                                        float initPos = num7;
                                        float xPos = num7;
                                        float yPos = TacticalGroupsSettings.ColonistBarPositionY;
                                        for (int groupID = 0; groupID < list.Count(); groupID++)
                                        {
                            ...
                            if (entries[j].colonyGroup.Map == Find.CurrentMap)
                            {
                                if (!WorldRendererUtility.WorldRenderedNow)
                                {
                                    List<PawnGroup> list = TacticUtils.GetAllPawnGroupFor(entries[j].colonyGroup);
                                    if (list.Any())
                                    {
                                        list.Reverse();
                                        float initPos = num7;
        ...
        private Vector2 GetDrawLoc(float groupStartX, float groupStartY, int group, int numInGroup, float scale)
        {
            List<TacticalColonistBar.Entry> entries = TacticUtils.TacticalColonistBar.Entries;
            int pawnCount = entries.Where(x => x.group == group && x.pawn != null).Count();
            if (TacticalGroupsSettings.OverridePawnRowCount && pawnCount / (float)(horizontalSlotsPerGroup[group]) > GetPawnRowCount(pawnCount))
            {
                float num = groupStartX + (numInGroup) % (horizontalSlotsPerGroup[group] + 1) * scale * (TacticalColonistBar.BaseSize.x + TacticalGroupsSettings.ColonistBarSpacingX);
                float y = groupStartY + (numInGroup) / (horizontalSlotsPerGroup[group] + 1) * scale * (TacticalColonistBar.BaseSize.y + TacticalGroupsSettings.ColonistBarSpacingY);
                return new Vector2(num, y);
            }
            else
            {
                float num = groupStartX + numInGroup % horizontalSlotsPerGroup[group] * scale * (TacticalColonistBar.BaseSize.x + TacticalGroupsSettings.ColonistBarSpacingX);
                float y = groupStartY + numInGroup / horizontalSlotsPerGroup[group] * scale * (TacticalColonistBar.BaseSize.y + TacticalGroupsSettings.ColonistBarSpacingY);
                if (numInGroup >= entriesInGroup[group] - entriesInGroup[group] % horizontalSlotsPerGroup[group])
                {
                    ...
                    num += num2 * scale * (TacticalColonistBar.BaseSize.x + TacticalGroupsSettings.ColonistBarSpacingX) * 0.5f;
                }
                return new Vector2(num, y);
            }
        }
