...
namespace TacticalGroups
{
    public class AttackMenu : TieredFloatMenu
    {
        ...
        protected override Vector2 InitialFloatOptionPositionShift => new Vector2(backgroundTexture.width / 10, 30f);
        ...
        {
            TieredFloatMenuOption option = new TieredFloatMenuOption(Strings.FireAtWill, null, Textures.LookBusyButton, Textures.LookBusyButtonHover, Textures.MenuButtonPress, TextAnchor.MiddleCenter,
                MenuOptionPriority.High, 0f, -1f, Strings.FireAtWillTooltip)
            {
                ...
                bottomIndent = Textures.MenuButton.height + 30
            };
            options.Add(option);
        }
        ...
        {
            TieredFloatMenuOption option = new TieredFloatMenuOption(Strings.Strongest, null, Textures.LookBusyButton, Textures.LookBusyButtonHover, Textures.MenuButtonPress, TextAnchor.MiddleCenter,
                MenuOptionPriority.High, 0f, -1f, Strings.StrongestTooltip)
            {
                action = delegate
                {
                    ...
                    colonistGroup.Draft();
                    colonistGroup.SelectAll();
                    colonistGroup.SwitchToAttackMode();
                    colonistGroup.RemoveOldLord();
                    List<Thing> targets = colonistGroup.Map.attackTargetsCache.GetPotentialTargetsFor(colonistGroup.ActivePawns.First()).Where(x => !x.Thing.Fogged()).Select(x => x.Thing).ToList();
                    LordMaker.MakeNewLord(Faction.OfPlayer, new LordJob_AssaultThingsStrongest(Faction.OfPlayer, targets, 0, true), colonistGroup.Map, colonistGroup.ActivePawns);
                    colonistGroup.SearchForJob();
                    CloseAllWindows();
                },
                bottomIndent = Textures.MenuButton.height + 10
            };
            options.Add(option);
        }
        ...
        {
            TieredFloatMenuOption option = new TieredFloatMenuOption(Strings.Weakest, null, Textures.LookBusyButton, Textures.LookBusyButtonHover, Textures.MenuButtonPress, TextAnchor.MiddleCenter,
                MenuOptionPriority.High, 0f, -1f, Strings.WeakestTooltip)
            {
                action = delegate
                {
                    ...
                    colonistGroup.RemoveOldLord();
                    List<Thing> targets = colonistGroup.Map.attackTargetsCache.GetPotentialTargetsFor(colonistGroup.ActivePawns.First()).Where(x => !x.Thing.Fogged()).Select(x => x.Thing).ToList();
                    LordMaker.MakeNewLord(Faction.OfPlayer, new LordJob_AssaultThingsWeakest(Faction.OfPlayer, targets, 0, true), colonistGroup.Map, colonistGroup.ActivePawns);
                    colonistGroup.SearchForJob();
                    CloseAllWindows();
                },
                bottomIndent = Textures.MenuButton.height + 35
            };
            options.Add(option);
        }
        ...
        {
            TieredFloatMenuOption option = new TieredFloatMenuOption(Strings.PursueFleeing, null, Textures.LookBusyButton, Textures.LookBusyButtonHover, Textures.MenuButtonPress, TextAnchor.MiddleCenter,
                MenuOptionPriority.High, 0f, -1f, Strings.PursueFleeingTooltip)
            {
                action = delegate
                {
                    ...
                    colonistGroup.RemoveOldLord();
                    List<Thing> targets = colonistGroup.Map.attackTargetsCache.GetPotentialTargetsFor(colonistGroup.ActivePawns.First()).Where(x => !x.Thing.Fogged()).Select(x => x.Thing).ToList();
                    LordMaker.MakeNewLord(Faction.OfPlayer, new LordJob_AssaultThingsPursueFleeing(Faction.OfPlayer, targets, 0, true), colonistGroup.Map, colonistGroup.ActivePawns);
                    colonistGroup.SearchForJob();
                    CloseAllWindows();
                },
                bottomIndent = Textures.MenuButton.height
            };
            options.Add(option);
        }
        ...
        public override void DoWindowContents(Rect rect)
        {
            ...
            Rect captureRect = new Rect(rect.x + 27, zero.y + 5, Textures.Arrest.width, Textures.Arrest.height);
            ...
            if (Mouse.IsOver(captureRect))
            {
                ...
                {
                    List<Pawn> victims = colonistGroup.Map.mapPawns.AllPawns.Where(x => x.RaceProps.Humanlike).ToList(); ;
                    foreach (Pawn pawn in colonistGroup.ActivePawns)
                    {
                        if (!pawn.Dead && !pawn.Downed && pawn.health.capacities.CapableOf(PawnCapacityDefOf.Manipulation))
                        {
                            for (int num = victims.Count - 1; num >= 0; num--)
                            {
                                ...
                                if (!victim.InBed() && !victim.mindState.WillJoinColonyIfRescued && pawn.CanReserveAndReach(victim, PathEndMode.OnCell, Danger.Deadly, 1, -1, null, ignoreOtherReservations: true))
                                {
                                    ...
                                    {
                                        Designation designation = victim.Map.designationManager.DesignationOn(victim);
            ...
            Rect executeRect = new Rect(captureRect.xMax + 15, captureRect.y, Textures.Execute.width, Textures.Execute.height);
            ...
            if (Mouse.IsOver(executeRect))
            {
                ...
                if (Event.current.type == EventType.MouseDown && Event.current.button == 0 && Event.current.clickCount == 1)
                {
                    List<Pawn> victims = colonistGroup.Map.mapPawns.AllPawns.Where(x => x.RaceProps.Humanlike && x.Downed && x.HostileTo(Faction.OfPlayer)).ToList();
                    foreach (Pawn pawn in colonistGroup.ActivePawns)
                    {
                        if (!pawn.Dead && !pawn.Downed && pawn.health.capacities.CapableOf(PawnCapacityDefOf.Manipulation))
                        {
