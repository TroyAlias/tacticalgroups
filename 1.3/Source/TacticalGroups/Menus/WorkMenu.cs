...
namespace TacticalGroups
{
    public class WorkMenu : TieredFloatMenu
    {
            ...
            : base(parentWindow, colonistGroup, originRect, backgroundTexture)
        {
            ...
            TieredFloatMenuOption takeFive = new TieredFloatMenuOption(Strings.TakeFive, null, Textures.LookBusyButton, Textures.LookBusyButtonHover, null, TextAnchor.MiddleCenter, MenuOptionPriority.High, 0f,
        ...
        public List<List<Texture2D>> GetBreakIconRows(int columnCount)
        {
            ...
            foreach (Texture2D icon in breakIconStates.Keys)
            {
                if (num == columnCount)
                {
        ...
        public override void DoWindowContents(Rect rect)
        {
            ...
            Rect getBreakIconRect = new Rect(rect.x + zero.x, rect.y + 475f, rect.width, rect.height);
            List<List<Texture2D>> iconRows = GetBreakIconRows(2);
                ...
                for (int j = 0; j < iconRows[i].Count; j++)
                {
                    ...
                    if (Mouse.IsOver(iconRect))
                    {
                        ...
                        if (Event.current.type == EventType.MouseDown && Event.current.button == 0 && Event.current.clickCount == 1)
                        {
                            ...
                            WorkSearchUtility.TakeABreak(breakIconStates[iconRows[i][j]], colonistGroup.ActivePawns);
                            Event.current.Use();
                        }
                    }
                }
            ...
            List<List<WorkTypeDef>> workRows = GetWorkTypeRows(2);
            Rect initialRect = new Rect(rect.x, rect.y + 75, 320, rect.height - 95);
            Rect workMenuRect = new Rect(initialRect);
            workMenuRect.height -= 103;
            Rect resetWorkPriorities = new Rect(workMenuRect.x + 20, rect.y + 32, Textures.ResetIcon.width, Textures.ResetIcon.height);
            GUI.DrawTexture(resetWorkPriorities, Textures.ResetIcon);
            if (Mouse.IsOver(resetWorkPriorities))
            {
                if (Event.current.type == EventType.MouseDown && Event.current.button == 0 && Event.current.clickCount == 1)
                {
                    colonistGroup.groupWorkPriorities?.Clear();
                    Event.current.Use();
                }
                TooltipHandler.TipRegion(resetWorkPriorities, Strings.GroupWorkPrioritiesReset);
            }

            Rect manualPrioritiesRect = new Rect(resetWorkPriorities.xMax + 10, rect.y + 30, 162, 40);
            ...
            for (int i = 0; i < workRows.Count; i++)
            {
                for (int j = 0; j < workRows[i].Count; j++)
                {
                    ...
                    KeyValuePair<WorkType, WorkState> workDictData = colonistGroup.activeWorkTypes.FirstOrDefault(x => x.Key.workTypeDef == workRows[i][j]);
                    WorkState workState;
                    ...
                    if (Mouse.IsOver(workTypeRect))
                    {
                        if (Event.current.type == EventType.MouseDown && Event.current.button == 0 && Event.current.clickCount == 1)
                        {
                            WorkSearchUtility.SearchForWork(workRows[i][j], colonistGroup.pawns);
                            colonistGroup.ChangeWorkState(workRows[i][j]);
                            Event.current.Use();
                        }
                    }
                    Rect workRect = new Rect(workTypeRect.xMax + 5, workTypeRect.y - 8, 24, 24);
                    DoWorkCell(workRect, workRows[i][j]);
                    DoWorkHeader(workRect, workRows[i][j]);
                }
            }
            Widgets.EndScrollView();
            DrawExtraGui(rect);

            Rect caravanButtonRect = new Rect(manualPrioritiesRect.xMax + 5, rect.y + 23, Textures.CaravanButton.width, Textures.CaravanButton.height);
            ...
            if (Mouse.IsOver(caravanButtonRect))
            {
                ...
                if (Event.current.type == EventType.MouseDown && Event.current.button == 0 && Event.current.clickCount == 1)
                {
                    ...
                    CaravanMenu floatMenu = new CaravanMenu(this, colonistGroup, windowRect, Textures.CaravanMenu);
            ...
            Rect researchMenuRect = new Rect(rect.x + 253, rect.y + 459, Textures.ResearchWorkButton.width, Textures.ResearchWorkButton.height);
        ...
        public void DoWorkCell(Rect rect, WorkTypeDef workType)
        {
            ...
            float y = rect.y + 2.5f;
            WidgetsWorkGroup.DrawWorkBoxFor(x, y, workType, colonistGroup);
            Text.Font = GameFont.Small;
        }
