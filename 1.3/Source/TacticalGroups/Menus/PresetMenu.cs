...
namespace TacticalGroups
{
    public class PresetMenu : TieredFloatMenu
    {
        ...
        public override void DoWindowContents(Rect rect)
        {
            ...
            Rect groupsLabel = new Rect(rect.x + 50, rect.y + 30, 80, 30);
            ...
            if (Mouse.IsOver(createNew))
            {
                ...
                if (Event.current.type == EventType.MouseDown && Event.current.button == 0 && Event.current.clickCount == 1)
                {
                    ...
                    GroupPreset newGroupPreset = new GroupPreset
                    {
                        id = TacticalGroupsSettings.AllGroupPresets.Count + 1
                    };
                    newGroupPreset.CopySettingsFrom(colonistGroup);
                    TieredFloatMenu floatMenu = new Dialog_NewPresetName(this, newGroupPreset, colonistGroup, windowRect, Textures.RenameTab, Strings.CreateNewPreset);
            ...
            if (Mouse.IsOver(resetGroup))
            {
                ...
                {
                    TacticDefOf.TG_SlideMenuOptionSFX.PlayOneShotOnCamera();
                    TieredFloatMenu floatMenu = new Dialog_ResetGroup(this, colonistGroup, windowRect, Textures.ResetTab);
                    OpenNewMenu(floatMenu);
                }
            }
            ...
            if (TacticalGroupsSettings.AllGroupPresets != null)
            {
                ...
                List<GroupPreset> groupPresets = Enumerable.Reverse(TacticalGroupsSettings.AllGroupPresets).ToList();
                for (int num = groupPresets.Count - 1; num >= 0; num--)
                {
                    GroupPreset preset = groupPresets[num];
                    ...
                    if (Mouse.IsOver(trashCan))
                    {
                        ...
                        if (Event.current.type == EventType.MouseDown && Event.current.button == 0 && Event.current.clickCount == 1)
                        {
                            TieredFloatMenu floatMenu = new Dialog_PresetRemove(this, preset, colonistGroup, windowRect, Textures.DeleteTab);
                            OpenNewMenu(floatMenu);
                        }
                    }
                    ...
                    if (Mouse.IsOver(savePreset))
                    {
                        ...
                        if (Event.current.type == EventType.MouseDown && Event.current.button == 0 && Event.current.clickCount == 1)
                        {
                            TieredFloatMenu floatMenu = new Dialog_PresetSave(this, preset, colonistGroup, windowRect, Textures.RenameTab, Strings.SavePreset);
                            OpenNewMenu(floatMenu);
                        }
                    }
                    ...
                    if (preset.groupWorkPriorities != null && preset.groupWorkPriorities.Any())
                    {
                        ...
                        foreach (KeyValuePair<WorkTypeDef, int> workPriority in preset.groupWorkPriorities)
                        {
                    ...
                    if (preset.activeWorkTypes != null && preset.activeWorkTypes.Any())
                    {
                        ...
                        foreach (KeyValuePair<WorkType, WorkState> activeWorkType in preset.activeWorkTypes)
                        {
                    ...
                    if (Mouse.IsOver(applyPreset))
                    {
                        ...
                        if (Event.current.type == EventType.MouseDown && Event.current.button == 0 && Event.current.clickCount == 1)
                        {
                            colonistGroup.ActivatePreset(preset);
                        }
                    }
                    pos.y += 35f;
                }
