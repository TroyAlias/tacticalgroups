...
namespace TacticalGroups
{
    public class MainFloatMenu : TieredFloatMenu
    {
        ...
        {
            TieredFloatMenuOption option = new TieredFloatMenuOption(Strings.Rally, null, Textures.RallyButton, Textures.RallyButtonHover, null, TextAnchor.MiddleCenter, MenuOptionPriority.High, 10f, -1f, Strings.RallyToolTip)
            {
                bottomIndent = 41,
                action = delegate
                {
        ...
        {
            TieredFloatMenuOption option = new TieredFloatMenuOption(Strings.Work, null, Textures.AOMButton, Textures.AOMButtonHover, Textures.AOMButtonPress, TextAnchor.MiddleLeft, MenuOptionPriority.High, 5f);
        ...
        public void AddWorkWindow(TieredFloatMenuOption option)
        {
            ...
            {
                Rect rect = new Rect(windowRect.x, windowRect.y + 30, windowRect.width, windowRect.height);
        ...
        {
            TieredFloatMenuOption option = new TieredFloatMenuOption(Strings.Orders, null, Textures.AOMButton, Textures.AOMButtonHover, Textures.AOMButtonPress, TextAnchor.MiddleLeft, MenuOptionPriority.High, 5f);
            option.action = delegate
            {
        ...
        {
            TieredFloatMenuOption option = new TieredFloatMenuOption(Strings.Manage, null, Textures.AOMButton, Textures.AOMButtonHover, Textures.AOMButtonPress, TextAnchor.MiddleLeft, MenuOptionPriority.High, 5f);
            option.action = delegate
            {
        ...
        public void AddManageWindow(TieredFloatMenuOption option)
        {
            ...
            {
                Rect rect = new Rect(windowRect.x, windowRect.y + 30, windowRect.width, windowRect.height);
                TieredFloatMenu floatMenu = new ManageMenu(this, colonistGroup, rect, (colonistGroup.isColonyGroup || colonistGroup.isTaskForce) ? Textures.ColonyManageDropMenu : Textures.ManageDropMenu);
                OpenNewMenu(floatMenu);
            }
        }
        ...
        public override void DrawExtraGui(Rect rect)
        {
            ...
            if (colonistGroup.entireGroupIsVisible || !colonistGroup.pawnIcons.Where(x => !x.Value.isVisibleOnColonistBar).Any())
            {
                if (Mouse.IsOver(iconRect))
                {
                    ...
                    if (Event.current.type == EventType.MouseDown && Event.current.button == 0 && Event.current.clickCount == 1)
                    {
                        ...
                        foreach (KeyValuePair<Pawn, PawnIcon> pawnIcon in colonistGroup.pawnIcons)
                        {
                            TacticUtils.TacticalGroups.visiblePawns.Remove(pawnIcon.Key);
                            pawnIcon.Value.isVisibleOnColonistBar = false;
                        }
                        colonistGroup.entireGroupIsVisible = false;
            ...
            else
            {
                if (Mouse.IsOver(iconRect))
                {
                    ...
                    if (Event.current.type == EventType.MouseDown && Event.current.button == 0 && Event.current.clickCount == 1)
                    {
                        ...
                        colonistGroup.entireGroupIsVisible = true;
                        TacticUtils.TacticalColonistBar.MarkColonistsDirty();
                        Event.current.Use();
                    }
                }
            ...
            Rect addPawnRect = new Rect(rect.x + (rect.width - Textures.AddPawnIcon.width) - 7f, rect.y + (rect.height - Textures.AddPawnIcon.height) - 7f, Textures.AddPawnIcon.width, Textures.AddPawnIcon.height);
        ...
        {
            base.PostClose();
            colonistGroup.Notify_WindowsClosed();
            CloseAllWindows();
        }
    }
}
