...
namespace TacticalGroups
{
    public class PawnGroupedMenu : TieredFloatMenu
    {
        ...
        public override void DoWindowContents(Rect rect)
        {
            ...
            Rect pawnsTitleBox = new Rect(rect.x + 75, rect.y + 35, 150, 30);
            Text.Font = GameFont.Medium;
            Widgets.Label(pawnsTitleBox, menuTitle);

            Rect pawnListingBox = new Rect(rect.x, 75, rect.width, rect.height - 95).ContractedBy(10);
    ...
    public class AnimalMenu : PawnGroupedMenu
    {
        ...
        protected override void DoPawnsListing(Rect rect)
        {
            ...
            IEnumerable<Pawn> animals = colonistGroup.Map.mapPawns.SpawnedPawnsInFaction(Faction.OfPlayer).Where(x => x.RaceProps.Animal);

            foreach (KeyValuePair<ThingDef, TreeNode_Pawns> data in animalNodes)
            {
            ...
            foreach (KeyValuePair<ThingDef, TreeNode_Pawns> listing in animalNodes)
            {
    ...
    public class PrisonerMenu : PawnGroupedMenu
    {
        ...
        protected override void DoPawnsListing(Rect rect)
        {
            ...
            List<Pawn> prisoners = colonistGroup.Map.mapPawns.PrisonersOfColony;

            foreach (KeyValuePair<PrisonerInteractionModeDef, TreeNode_Pawns> data in prisonerNodes)
            {
                data.Value.pawns?.Clear();
            }
            ...
            foreach (KeyValuePair<PrisonerInteractionModeDef, TreeNode_Pawns> listing in prisonerNodes)
            {
    ...
    public class GuestMenu : PawnGroupedMenu
    {
        ...
        protected override void DoPawnsListing(Rect rect)
        {
            ...
            IEnumerable<Pawn> guests = colonistGroup.Map.mapPawns.AllPawnsSpawned.Where(x => x.RaceProps.Humanlike && !x.IsPrisoner && !x.Fogged() && x.Faction != null &&
            x.Faction != Faction.OfPlayer && !x.Faction.HostileTo(Faction.OfPlayer));

            foreach (KeyValuePair<FactionDef, TreeNode_Pawns> data in guestNodes)
            {
                data.Value.pawns?.Clear();
            }
            ...
            foreach (KeyValuePair<FactionDef, TreeNode_Pawns> listing in guestNodes)
            {
    ...
    public class SlaveMenu : PawnGroupedMenu
    {
        ...
        protected override void DoPawnsListing(Rect rect)
        {
            ...
            IEnumerable<Pawn> slaves = colonistGroup.Map.mapPawns.AllPawnsSpawned.Where(x => x.RaceProps.Humanlike && x.IsSlaveOfColony && !x.Fogged());
