...
namespace TacticalGroups
{
    public class CaravanMenu : TieredFloatMenu
    {
        ...
        {
            base.PostOpen();
            caravanOptionsMenu = new CaravanOptionsMenu(this, colonistGroup, windowRect, Textures.LoadoutMenu);
        ...
        public override void DoWindowContents(Rect rect)
        {
            ...
            Rect sendRect = new Rect(rect.x + 20, rect.y + 25, Textures.SetClearButton.width, Textures.SetClearButton.height);
            ...
            if (Mouse.IsOver(sendRect))
            {
                if (Event.current.type == EventType.MouseDown && Event.current.button == 0 && Event.current.clickCount == 1)
                {
                    ...
                    Dialog_FormCaravan window = new Dialog_FormCaravan(colonistGroup.Map);
                    Find.WindowStack.Add(window);
                    Traverse.Create(window).Field("autoSelectFoodAndMedicine").SetValue(colonistGroup.travelSuppliesEnabled);
                    List<Pawn> selectedPawns = new List<Pawn>();
                    foreach (TransferableOneWay trad in window.transferables)
                    {
                        if (trad.AnyThing is Pawn pawn2 && colonistGroup.ActivePawns.Contains(pawn2))
                        {
                            trad.AdjustTo(1);
                    ...
                    if (!colonistGroup.travelSuppliesEnabled && colonistGroup.bedrollsEnabled)
                    {
                        SelectBedrolls(window, window.transferables, selectedPawns);
                    }
                    CloseAllWindows();
                    Event.current.Use();
                }
            ...
            Rect unloadRect = new Rect(sendRect.xMax - 15, sendRect.yMax + 20, Textures.SetClearButton.width, Textures.SetClearButton.height);
            ...
            if (Mouse.IsOver(unloadRect))
            {
                ...
                {
                    TacticDefOf.TG_ClickSFX.PlayOneShotOnCamera();
                    colonistGroup.AssignTemporaryWorkers(WorkTypeEnum.UnloadCaravan);
                    WorkSearchUtility.SearchForWork(WorkTypeEnum.UnloadCaravan, colonistGroup.ActivePawns);
                    Event.current.Use();
                }
